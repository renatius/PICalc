...
namespace PovertyIndex.DomainModel
{
    ...
    public class Person
    {
        ...
        public void AddObservation(int year, bool isPoor, double povertyGap)
        {
            ...
            {
                string errorFormat =
                    "({0}, {1}) An observation for year {2} has been already recorded";
                string errorMessage =
                    string.Format(errorFormat, Country, PersonId, year);

                throw new BusinessException(errorMessage);
            }

            observations[year] = new PovertyStatus() { Year = year, IsPoor = isPoor, PovertyGap = povertyGap };
        ...
        private Dictionary<int, PovertyStatus> observations;
    }
}
